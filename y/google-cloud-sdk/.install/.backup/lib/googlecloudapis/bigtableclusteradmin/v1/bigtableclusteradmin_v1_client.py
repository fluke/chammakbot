...
from googlecloudapis.bigtableclusteradmin.v1 import bigtableclusteradmin_v1_messages as messages
...
class BigtableclusteradminV1(base_api.BaseApiClient):
  ...
  class OperationsService(base_api.BaseApiService):
    ...
    def Cancel(self, request, global_params=None):
      """Starts asynchronous cancellation on a long-running operation. The server makes a best effort to cancel the operation, but success is not guaranteed. If the server doesn't support this method, it shall return `google.rpc.Code.UNIMPLEMENTED`. Clients may use [Operations.GetOperation][google.longrunning.Operations.GetOperation] or other methods to check whether the cancellation succeeded or the operation completed despite cancellation.
    ...
    def Delete(self, request, global_params=None):
      """Deletes a long-running operation. It indicates the client is no longer interested in the operation result. It does not cancel the operation. If the server doesn't support this method, it shall return `google.rpc.Code.UNIMPLEMENTED`.
    ...
    def List(self, request, global_params=None):
      """Lists operations that match the specified filter in the request. If the server doesn't support this method, it shall return `UNIMPLEMENTED`. NOTE: the `name` binding below allows API services to override the binding to use different resource name schemes, such as `users/*/operations`.
