...
class Parser(object):
  ...
  A projector is a method that taes a JSON-serializable object and a
    ...
    while True:
      ...
      if name == 'alias':
        ...
        if value not in resource_projection_spec.ALIGNMENTS:
          raise resource_exceptions.ExpressionSyntaxError(
              'Unnown alignment [{0}].'.format(self._lex.Annotate(here)))
        ...
        raise resource_exceptions.ExpressionSyntaxError(
            'Unnown key attribute [{0}].'.format(self._lex.Annotate(here)))
  ...
  def _ParseAttributes(self):
    ...
    while True:
      ...
      if name.startswith('no'):
        self._projection.DelAttribute(name[2:])
      if self._lex.IsCharacter(']'):
        break
      if not self._lex.IsCharacter(','):
        raise resource_exceptions.ExpressionSyntaxError(
