...
class InstanceGroupListInstances(base_classes.BaseCommand):
  ...
  def Run(self, args):
    ...
    items = lister.ProcessResults(
        resources=list(_UnwrapResponse(responses, 'items')),
  ...
  def _GetResources(self, args):
    ...
    request = self.service.GetRequestType('ListInstances')(
    ...
    results = list(request_helper.MakeRequests(
        requests=[(self.service, 'ListInstances', request)],
        http=self.http,
...
class InstanceGroupSetNamedPorts(base_classes.NoOutputAsyncMutator):
  ...
  def Args(parser):
    ...
    parser.add_argument(
        ...
        metavar='NAME',
        help='The names of the ports. The number of names must match '
        'the number of provided ports.')

    parser.add_argument(
        '--ports',
        required=True,
        type=arg_parsers.ArgList(element_type=int, min_length=1),
        action=arg_parsers.FloatingListValuesCatcher(),
        metavar='PORT',
        help='The port numbers. The number of ports must match '
  ...
  def CreateRequests(self, args):
    ...
    if len(args.names) != len(args.ports):
      raise exceptions.InvalidArgumentException(
          'ports', 'Number of ports must match number of names.')

    ports = []
    for host, port in zip(args.names, args.ports):
      ports.append(self.messages.NamedPort(name=host, port=port))
  ...
  detailed_help = {
      'brief': 'Adds named ports to an instance group',
      'DESCRIPTION': """\
          Named ports are key:value pairs metadata representing
          the service name and the port that it's running on. Named ports
          ...
          *{command}* adds named ports (name and port) to all instances in
          an instance group.
          """,
      'EXAMPLES': """\
          For example, to apply the named ports to an entire instance group:

            $ {command} example-instance-group --names example-service \
                --port=1111 \
                --zone us-central1-a
