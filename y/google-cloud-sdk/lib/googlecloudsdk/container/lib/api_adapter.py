...
class V1beta1Adapter(APIAdapter):
  ...
  def Zone(self, cluster_ref):
    return cluster_ref.zoneId

  def Version(self, cluster):
    return cluster.clusterApiVersion

  def PrintClusters(self, clusters):
    list_printer.PrintResourceList(
        'containerv1beta1.projects.zones.clusters', clusters)

  def PrintOperations(self, operations):
    list_printer.PrintResourceList(
        'containerv1beta1.projects.zones.operations', operations)
  ...
  def CreateCluster(self, cluster_ref, options):
    ...
    if options.node_machine_type:
      node_config.machineType = options.node_machine_type
    if options.node_source_image:
      node_config.sourceImage = options.node_source_image
    if options.scopes:
      node_config.serviceAccounts = self._CreateServiceAccount(options.scopes)
    ...
    if options.cluster_version:
      create_cluster_req.cluster.clusterApiVersion = options.cluster_version
    if options.network:
      create_cluster_req.cluster.network = options.network
    if options.container_ipv4_cidr:
      create_cluster_req.cluster.containerIpv4Cidr = options.container_ipv4_cidr
  ...
  def _CreateServiceAccount(self, scopes):
    return [
        self.messages.ServiceAccount(
            email='default',
            scopes=sorted(ExpandScopeURIs(scopes)))
    ]
  ...
  def ListClusters(self, project, zone=None):
    if zone:
      ...
      # Global list
      req = self.messages.ContainerProjectsClustersListRequest(
          projectId=project)
      return self.client.projects_clusters.List(req)
  ...
  def ListOperations(self, project, zone=None):
    if zone:
      ...
      # Global list
      req = self.messages.ContainerProjectsOperationsListRequest(
          projectId=project)
      return self.client.projects_operations.List(req)
