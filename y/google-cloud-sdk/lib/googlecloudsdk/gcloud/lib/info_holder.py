...
class LogsInfo(object):
  ...
  def GetRecentRuns(self):
    ...
    Returns:
      A list of tuples of the form ([path to log file], [command run]).
    """
    recent_runs = []
    for log_path in self.last_logs:
      with open(log_path, 'r') as log_file:
        pattern = re.compile(r'Running (gcloud\.[a-z.]+)')
        match = pattern.search(log_file.read())
        if match:
          # ex. gcloud.group.subgroup.command
          dotted_cmd_string, = match.groups()
          cmd = ' '.join(dotted_cmd_string.split('.'))
          recent_runs.append((log_path, cmd))
    return recent_runs
