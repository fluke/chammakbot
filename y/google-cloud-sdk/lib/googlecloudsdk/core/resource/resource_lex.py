  ...
  while lex.SkipSpace():
    ...
    if not key:
      ...
      raise resource_exceptions.ExpressionSyntaxError(
          'key expected [%s].' % lex.Annotate(here))
    ...
    if not operator:
      ...
      if lex.IsString('AND'):
        ...
        raise resource_exceptions.ExpressionSyntaxError(
            'Operator expected [%s].' % lex.Annotate())
    ...
    if not operand:
      raise resource_exceptions.ExpressionSyntaxErrorSyntaxError(
          'Operand expected [%s].' % lex.Annotate())
...
class Lexer(object):
  ...
  Attributes:
    ...
    _position: The _expr index of the next expression character to parse.
  ...
  def IsCharacter(self, characters, peek=False, eoi_ok=False):
    ...
    if self.EndOfInput():
      if peek or eoi_ok:
        return None
      raise resource_exceptions.ExpressionSyntaxError(
          'More tokens expected [%s].' % self.Annotate())
  ...
  def IsString(self, name, peek=False):
    ...
    Args:
      ...
      peek: Dooes not consume the string on match if True.
  ...
  def Token(self, terminators='', space=True, convert=False):
    ...
    Returns:
      The token, None if there is none.
    """
    ...
    token = None  # The token string, None for no token, '' for empty token.
    i = self.GetPosition()
    while True:
      ...
      if c == self._ESCAPE and not self.EndOfInput(i + 1):
        ...
        if token is None:
          token = ''
        if c == self._ESCAPE or c == quote or not quote and c in self._QUOTES:
          token += c
        else:
          token += self._ESCAPE + c
        i += 1
      elif c == quote:
  ...
  def Args(self, convert=False):
    ...
    while True:
      ...
      if arg is not None:
        ...
        raise resource_exceptions.ExpressionSyntaxError(
            'Argument expected [%s].' % self.Annotate(here))
      if end:
        break
      if not self.IsCharacter(','):
        raise resource_exceptions.ExpressionSyntaxError(
            'Closing ) expected in argument list [%s].' % self.Annotate(here))
      required = True
    return args
  ...
  def Key(self):
    ...
    while not self.EndOfInput():
      ...
      if name:
        ...
        raise resource_exceptions.ExpressionSyntaxError(
            'Non-empty key name expected [%s].' % self.Annotate(here))
      if self.EndOfInput():
        break
      if self.IsCharacter(']'):
        raise resource_exceptions.ExpressionSyntaxError(
            'Unmatched ] in key [%s].' % self.Annotate(here))
      if self.IsCharacter('['):
      ...
      if self.EndOfInput():
        raise resource_exceptions.ExpressionSyntaxError(
            'Non-empty key name expected [%s].' % self.Annotate())
    return key
